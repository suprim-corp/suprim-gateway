...
class MessagesController {
    ...
    MessagesController(KiroHttpClient kiroClient, PayloadBuilder payloadBuilder,
    ...
    void messages(@RequestBody Map<String, Object> request,
        ...
        if (key != null && !rateLimiter.isAllowed(key.id(), key.rateLimitPerMin())) {
            ...
            httpRes.getWriter().write("{\"type\":\"error\",\"error\":{\"type\":\"rate_limit_error\",\"message\":\"Rate limit exceeded\"}}");
            return;
        }
        ...
        try {
            List<Map<String, Object>> openAiMessages = convertAnthropicToOpenAi(request);
            HashMap<String, Object> openAiReq = new HashMap<>();
            openAiReq.put("messages", openAiMessages);
            openAiReq.put("model", model);
            if (request.containsKey("tools")) openAiReq.put("tools", request.get("tools"));
            ...
            if (response.status() != 200) {
                ...
                httpRes.setContentType("application/json");
                httpRes.getWriter().write("{\"type\":\"error\",\"error\":{\"type\":\"api_error\",\"message\":\"Upstream error\"}}");
                return;
            }
            ...
            if (stream) {
                ...
                writer.write("event: message_start\ndata: " + mapper.writeValueAsString(Map.of(
                        "type", "message_start",
                        "message", Map.of("id", msgId, "type", "message", "role", "assistant", "content", List.of(), "model", model)
                )) + "\n\n");

                writer.write("event: content_block_start\ndata: " + mapper.writeValueAsString(Map.of(
                        "type", "content_block_start", "index", blockIdx, "content_block", Map.of("type", "text", "text", "")
                )) + "\n\n");
                writer.flush();
                    ...
                    for (KiroEvent event : events) {
                        if ("content".equals(event.type()) && event.content() != null) {
                            writer.write("event: content_block_delta\ndata: " + mapper.writeValueAsString(Map.of(
                                    "type", "content_block_delta", "index", blockIdx, "delta", Map.of("type", "text_delta", "text", event.content())
                            )) + "\n\n");
                            writer.flush();
                            totalTokens += Math.max(1, event.content().length() / 4);
                        }
                    }
                }
                ...
                writer.write("event: message_delta\ndata: " + mapper.writeValueAsString(Map.of(
                        "type", "message_delta", "delta", Map.of("stop_reason", "end_turn"),
                        "usage", Map.of("output_tokens", totalTokens)
                )) + "\n\n");
                writer.write("event: message_stop\ndata: {\"type\":\"message_stop\"}\n\n");
                writer.flush();

                int latency = (int) (System.currentTimeMillis() - startTime);
                logService.log(keyId, null, model, model, 200, null, totalTokens, latency, null, true, clientIp(httpReq), null);
                ...
                for (KiroEvent event : events) {
                    if ("content".equals(event.type()) && event.content() != null) content.append(event.content());
                }
                ...
                Map<String, Object> result = Map.of(
                        ...
                        "type", "message",
                        "role", "assistant",
                        "content", List.of(Map.of("type", "text", "text", content.toString())),
                        "model", model,
                        "stop_reason", "end_turn",
                        "usage", Map.of("input_tokens", 0, "output_tokens", outputTokens)
                );
                httpRes.setContentType("application/json");
                mapper.writeValue(httpRes.getWriter(), result);

                int latency = (int) (System.currentTimeMillis() - startTime);
                logService.log(keyId, null, model, model, 200, null, outputTokens, latency, null, false, clientIp(httpReq), null);
            }
        } catch (Exception e) {
            ...
            if (!httpRes.isCommitted()) {
                ...
                httpRes.setContentType("application/json");
                httpRes.getWriter().write("{\"type\":\"error\",\"error\":{\"type\":\"api_error\",\"message\":\"Internal server error\"}}");
            }
        }
    }
        ...
        if (request.containsKey("system")) {
            ...
            if (sys instanceof String s) {
                ...
                for (Object item : list) {
                    if (item instanceof Map<?, ?> m && m.containsKey("text")) sb.append(m.get("text"));
                }
