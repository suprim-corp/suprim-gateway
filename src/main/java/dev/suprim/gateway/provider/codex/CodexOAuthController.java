...
public class CodexOAuthController {
	...
	String agentScript(String state, HttpServletRequest httpReq) {
		...
		return "#!/bin/bash\n"
		       ...
		       + "echo ''\n"
		       + "printf '  Opening browser for OpenAI login...\\n'\n"
		       + "echo ''\n"
		       + "\n"
		       + "open_url() {\n"
		       ...
		       + "\n"
		       + "# Simple netcat-based callback listener\n"
		       + "{\n"
		       + "  while true; do\n"
		       + "    RESPONSE=$(echo -e 'HTTP/1.1 302 Found\\r\\nLocation: '$GATEWAY'/providers?codex=connected\\r\\nConnection: close\\r\\n\\r\\n' | nc -l 1455 2>/dev/null)\n"
		       + "    CODE=$(echo \"$RESPONSE\" | grep -o 'code=[^& ]*' | cut -d= -f2)\n"
		       + "    if [ -n \"$CODE\" ]; then\n"
		       + "      echo \"$CODE\" > /tmp/.codex_auth_code\n"
		       + "      break\n"
		       + "    fi\n"
		       + "  done\n"
		       + "} &\n"
		       ...
		       + "\n"
		       + "# Wait for code\n"
		       + "while [ ! -f /tmp/.codex_auth_code ]; do sleep 1; done\n"
		       + "CODE=$(cat /tmp/.codex_auth_code)\n"
		       + "rm -f /tmp/.codex_auth_code\n"
		       + "\n"
		       + "echo 'Authorized! Exchanging token...'\n"
