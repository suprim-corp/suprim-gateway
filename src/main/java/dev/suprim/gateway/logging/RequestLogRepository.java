...
class RequestLogRepository {
		...
		jdbc.update(
				"""
						INSERT INTO request_logs (id, virtual_key_id, account_id, model, requested_model, status,
						    ...
						    streaming, client_ip, error_message, credits, created_at)
						VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
						""",
				log.id(),
				log.virtualKeyId(),
	...
	int countErrorsSince(long since) {
		Integer result = jdbc.queryForObject(
				"SELECT COUNT(*) FROM request_logs WHERE created_at >= ? AND status >= 400",
				Integer.class,
				since
		);
		return result != null ? result : 0;
	...
	long sumTokensSince(long since) {
		Long result = jdbc.queryForObject(
				"SELECT COALESCE(SUM(total_tokens), 0) FROM request_logs WHERE created_at >= ?",
				Long.class,
				since
		);
		return result != null ? result : 0;
