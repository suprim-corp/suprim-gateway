...
class VirtualKeyRepository {
    ...
    private final RowMapper<VirtualKey> mapper = (rs, rowNum) -> new VirtualKey(
            ...
            rs.getObject("budget_tokens") != null ? rs.getInt("budget_tokens") : null,
            rs.getObject("budget_requests") != null ? rs.getInt("budget_requests") : null,
            rs.getObject("budget_cost") != null ? rs.getInt("budget_cost") : null,
            rs.getInt("period_tokens_used"),
            rs.getInt("period_requests_used"),
            rs.getObject("period_reset_at") != null ? rs.getLong("period_reset_at") : null,
            rs.getInt("total_requests"),
            rs.getInt("total_tokens"),
            rs.getObject("last_used_at") != null ? rs.getLong("last_used_at") : null,
    ...
    VirtualKey findByKeyHash(String hash) {
        List<VirtualKey> results = jdbc.query("SELECT * FROM virtual_keys WHERE key_hash = ?", mapper, hash);
        return results.isEmpty() ? null : results.getFirst();
    }

    VirtualKey findById(String id) {
        List<VirtualKey> results = jdbc.query("SELECT * FROM virtual_keys WHERE id = ?", mapper, id);
        return results.isEmpty() ? null : results.getFirst();
    }

    List<VirtualKey> findAll(int limit, int offset) {
        return jdbc.query("SELECT * FROM virtual_keys ORDER BY created_at DESC LIMIT ? OFFSET ?", mapper, limit, offset);
    }

    int count() {
        Integer result = jdbc.queryForObject("SELECT COUNT(*) FROM virtual_keys", Integer.class);
    ...
    void insert(VirtualKey key) {
        jdbc.update("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                key.id(), key.name(), key.keyHash(), key.keyPrefix(), key.accountId(),
                key.enabled() ? 1 : 0, key.revokedAt(), key.rateLimitPerMin(), key.allowedModels(),
                key.budgetPeriod(), key.budgetTokens(), key.budgetRequests(), key.budgetCost(),
                key.periodTokensUsed(), key.periodRequestsUsed(), key.periodResetAt(),
                key.totalRequests(), key.totalTokens(), key.lastUsedAt(), key.createdAt());
    }

    void updateEnabled(String id, boolean enabled) {
        jdbc.update("UPDATE virtual_keys SET enabled = ? WHERE id = ?", enabled ? 1 : 0, id);
    }

    void revoke(String id) {
        jdbc.update("UPDATE virtual_keys SET revoked_at = ?, enabled = 0 WHERE id = ?", System.currentTimeMillis(), id);
    }

    void updateBudget(String id, String period, Integer tokens, Integer requests, Integer cost) {
        jdbc.update("UPDATE virtual_keys SET budget_period = ?, budget_tokens = ?, budget_requests = ?, budget_cost = ? WHERE id = ?",
                period, tokens, requests, cost, id);
    }
